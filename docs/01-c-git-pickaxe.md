...
3. Wie findest du den Commit, in dem der String `DEBUG start calc` erstmals in `src/cli.ts` eingefügt wurde, und zeigst den Diff/Patch dazu an? *Tipp:* Für die Beantwortung dieser Frage wird die Option `--reverse` benötigt. Warum ist das so? Was passiert ohne diese Optionen?
