# Übungsblatt: Git Pickaxe (Gruppe D)

Für die folgenden Übungen wird angenommen, dass sich der Benutzer im `main`-Branch befindet.

1. Wie findest du den Commit, in dem der String `DEBUG start calc` erstmals in `src/cli.ts` eingefügt wurde, und zeigst den Diff/Patch dazu an? *Tipp:* Für die Beantwortung dieser Frage wird die Option `--reverse` benötigt. Warum ist das so? Was passiert ohne diese Optionen?
...
2. Zeige alle Commits an in denen der String "adds positive" oder "adds negtive" vorkommt. Zeige zusätzlich zu den Commit-Metadaten auch den Diff der betreffenden Dateien an. *Tipp:* Nutze zur Lösung dieser Aufgabe den RegEx `"adds ("positives"|"negatives")"`. Benutze Google um herauszufinden, wie man die Pickaxe mit RegEx benutzen kann.

<details><summary>Antwort</summary>
<code><pre>
  git log -G "adds ("positives"|"negatives")" -p
</pre></code>
Während die Option <code>-S</code> nur nach Strings sucht, kann <code>-G</code> RegEx verarbeiten.
</details>

3. In welchem Commit wurde die Datei `./src/divide.ts` 

    a. dem Repository hinzugefügt? 
    
    b. im Repository verändert? 
    
    Zeige den Commit als Oneliner an. *Tipp:* Google die Optionen `--diff-filter` und `--oneline`.
...
<code><pre>
  git log --diff-filter=M --oneline -- src/divide.ts
</pre></code>
</details>

4. Wechsle in den Branch `main`. Zeige nun die letzten beiden Commits des Autors *JohnDoe*, die `src/divide.ts` verändert haben, als Oneliner. *Tipp:* Google nach einer Option, die nach dem Autor filtern kann. Wie verändert sich die Antwort, wenn die Option `--source` hinzugefügt wird?
...
5. Worin unterscheiden sich die beiden Kommandos?
